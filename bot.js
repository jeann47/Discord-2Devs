...
const js = new Discord.MessageEmbed()
  ...
  .setImage(
    'https://www.mememaker.net/api/bucket?path=static/img/memes/full/2019/Jan/8/3/pennywise-nodejs-212.png'
  )
  .setTimestamp();
...
client.on('message', async (message) => {
  ...
  if (command === 'buscar') {
    try {
      ...
      message.channel
        .send(file)
        .then(() => message.delete({ timeout: 150000 }));
    } catch (err) {
      message.channel
  ...
  if (
    ...
    message.content.includes('PHP') ||
    message.content.includes('pêagapê') ||
    message.content.includes('PHFODASE')
  ) {
    message.react('💩');
    message.react('🖕');
    message.react('❌');
  ...
  if (
    ...
    message.content.includes('JS') ||
    message.content.includes('linguagem boa')
  ) {
